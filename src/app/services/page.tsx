            ...
            {servicesList.map((srv, index) => {
              ...
              return (
                <div 
                  key={srv.id} 
                  id={srv.id}
                  className="rounded-3xl border border-slate-200/80 bg-white p-8 md:p-12 shadow-xl hover:shadow-2xl transition-all"
                >
                  <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
                    ...
                    <div className="lg:col-span-7">
                      <div className="flex items-center gap-3 mb-4">
                        ...
                        <span className="px-3 py-1 rounded-full bg-slate-100 text-xs font-mono font-bold text-slate-700 uppercase">
                          {srv.badge}
                        </span>
                      </div>

                      <h2 className="text-3xl font-black text-slate-900">{srv.title}</h2>
                      <p className="mt-4 text-base text-slate-600 leading-relaxed font-normal">
                        {srv.description}
                      </p>

                      <div className="mt-8">
                        <h4 className="text-xs font-black uppercase tracking-wider text-slate-400 mb-3">Key Technical Specifications</h4>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                          {srv.specs.map((sp, i) => (
                            <div key={i} className="flex items-center gap-2 text-xs font-bold text-slate-800 bg-slate-50 px-3 py-2 rounded-xl border border-slate-100">
                    ...
                    <div className="lg:col-span-5 bg-slate-950 p-6 md:p-8 rounded-2xl text-white shadow-xl border border-slate-800">
                      ...
                      <div className="mt-8 pt-6 border-t border-slate-800 flex items-center justify-between">
                        <span className="text-xs text-slate-400">Ready for Global Deployment</span>
                        <Link
                          href="/contact"
