...
export default function ProductsPage() {
  return (
    <>
            ...
            <Reveal key={p.name} delay={(i % 2) * 0.08}>
              <SpotlightCard accent={ACCENTS[i % ACCENTS.length]} className="h-full">
                <div className="p-8 md:p-9 flex flex-col h-full">
                  ...
                  <p className="text-ink/60 dark:text-white/60 leading-relaxed flex-1">{p.description}</p>
                  <Link
                    href="/contact"
                    className="inline-flex items-center mt-7 pt-6 border-t border-ink/5 dark:border-white/10 text-ink dark:text-white font-semibold text-sm gap-1.5 group-hover:gap-2.5 transition-all"
                  >
      ...
      <section className="py-24 bg-mist dark:bg-[#07090E]">
          ...
          <Reveal className="rounded-3xl bg-brand-gradient-soft border border-ink/8 dark:border-white/10 px-8 py-16 lg:px-16 text-center">
            ...
            <div className="mt-8 flex justify-center">
              <Magnetic>
                <Link
                  href="/services"
                  className="inline-flex items-center justify-center rounded-full bg-ink text-white px-8 py-4 text-[15px] font-semibold hover:bg-ink-soft dark:bg-white dark:text-ink dark:hover:bg-slate-200 transition-colors shadow-sm"
                >
                  See custom services
                </Link>
              </Magnetic>
