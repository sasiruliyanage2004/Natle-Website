...
export default function AIEnterpriseROICalculator() {
 ...
 isSelected
 ? "bg-emerald-500/15 border-emerald-500 text-slate-900 shadow-md"
 : "bg-white/40 border-slate-200/80 text-slate-700 hover:border-emerald-500/40"
 }`}
 >
 <span className="text-xl">{item.icon}</span>
 <div>
 <span className="text-xs font-bold block leading-tight">{item.name}</span>
 <span className="text-[10px] font-mono text-[#059669] ]">-{item.workloadDropPct}% Workload Drop</span>
 </div>
 </button>
 );
 })}
 ...
 <span className="text-lg font-black font-mono text-[#059669] ] bg-emerald-500/10 px-3 py-1 rounded-xl border border-emerald-500/20">
 ...
 className="w-full h-2.5 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-[#10E599]"
 />
 ...
 ? "bg-emerald-500/15 border-emerald-500 text-slate-900 "
 ...
 <span className="text-[10px] text-slate-500 block">100% Zero Network Egress</span>
 </button>
 ...
 <span className="text-[10px] text-slate-500 block">Managed Multi-Region VPC</span>
 </button>
 </div>
 </div>
 ...
 viewport={{ once: true }}
 transition={{ duration: 0.55 }}
 className="lg:col-span-6 bg-slate-950 ] rounded-3xl p-8 md:p-10 text-white shadow-2xl border border-slate-800 flex flex-col justify-between"
 >
 <div>
